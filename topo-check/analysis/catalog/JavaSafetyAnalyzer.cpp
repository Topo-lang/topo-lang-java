// JavaSafetyAnalyzer -- L2 whitelist-based containment analysis engine.
//
// Uses JDT semantic tokens to find call sites, resolves each target
// via hover, and checks qualified names against JavaSafePatterns whitelist.
...
//
// Java-specific differences from CppSafetyAnalyzer:
// - No preprocessor scanner (Java has no macros)
// - Converts '.' to '::' in qualified names for internal lookups
...
/// Convert Java dot-separated qualified name to Topo internal :: separator.
/// e.g., "java.util.List" -> "java::util::List"
std::string dotToColonSep(const std::string& name) {
    std::string result;
    result.reserve(name.size() + name.size() / 4); // rough estimate
    for (char c : name) {
        if (c == '.') {
            result += "::";
        } else {
...
/// JDT hover responses vary but commonly include:
///   "java.util.List<E>"              -> "java.util.List"
///   "public void MyClass.method()"   -> "MyClass.method"
///   "int java.lang.Math.abs(int)"    -> "java.lang.Math.abs"
...
JavaSafetyAnalyzer::JavaSafetyAnalyzer(lsp::JdtBridge& bridge, const JavaSafePatterns& patterns)
    : bridge_(bridge), patterns_(patterns) {}

CheckResult JavaSafetyAnalyzer::analyze(const SymbolTable& symbols,
    ...
    if (!bridge_.isAvailable()) {
        ...
        d.message = "JDT Language Server unavailable -- falling back to L1 regex scanning";
    ...
    // Principle 16: if JDT produced no tokens for any file, do not pretend
    ...
    if (!sourceFiles.empty() && filesWithEmptyTokens == static_cast<int>(sourceFiles.size())) {
        ...
        d.severity = Severity::Warning;
        d.check = "containment-l2";
        d.message = "JDT Language Server returned no semantic tokens for any of " +
    ...
    // Java has no macros, so no preprocessor scanner pass is needed.
    ...
    if (filesWithEmptyTokens > 0) {
        ...
        d.severity = Severity::Warning;
        d.check = "containment-l2";
        d.message = "JDT Language Server returned no semantic tokens for " +
...
bool JavaSafetyAnalyzer::analyzeFile(const std::string& filePath,
    ...
    // 2. Get semantic tokens (retry: jdtls may need time to analyze after didOpen,
    // mirroring the CppSafetyAnalyzer clangd defensive block)
    ...
    for (const auto& token : tokens) {
        ...
        // Convert Java dot notation to :: for internal lookups
        std::string internalName = dotToColonSep(qualifiedName);

        // Check if this is a safe stdlib call
        if (patterns_.isStdlibSymbolSafe(internalName)) continue;
        ...
        for (const auto& [name, fn] : symbols.functions()) {
            if (fn.qualifiedName == qualifiedName || fn.simpleName == qualifiedName ||
                fn.qualifiedName == internalName || fn.simpleName == internalName) {
                isDeclared = true;
                break;
            }
        }
        ...
        // Resolve the enclosing method via documentSymbol so external
        // functions are recognized by isExternalCaller(). The synthetic
        // placeholder survives only as a last-resort fallback when the
        // outline is empty — that keeps any residual failure path
        // visible rather than silently misclassifying the call.
        ...
        // Classify via JavaUnsafeCatalog; default to System for unknown external calls
        auto catalogLevel = JavaUnsafeCatalog::classifyCall(qualifiedName);
